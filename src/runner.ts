...
export class TaskRunner {
  ...
  private async buildSystemPrompt(isHeartbeat?: boolean): Promise<string> {
    ...
    parts.push(
      ...
      "",
      "## X (Twitter)",
      "- You can post tweets using the x_post tool. Max 280 characters per tweet.",
      "- To reply to a tweet: use x_reply with the tweet ID and your reply text.",
      "- To check engagement on a tweet: use x_get_tweet to see likes, retweets, replies, and impressions.",
      "- To search recent tweets: use x_search with a query (keywords, @mentions, #hashtags).",
      "- To read a user's timeline: use x_timeline with their user ID.",
      "- To check your own X account: use x_me to get your user ID and username.",
      "- If an X tool call fails, retry once before reporting the error.",
      "",
      "## Farcaster",
      "- You can post to ANY Farcaster channel — no config permission or allowed_channels list is needed.",
      "- Your Farcaster credentials (NEYNAR_API_KEY, FARCASTER_SIGNER_UUID, FARCASTER_FID) are already in .env. Do not ask the founder for them — just use post_cast, read_channel, reply_to_cast, etc. directly.",
      "- If a Farcaster tool call fails, retry once before reporting the error.",
      "",
      "## Tool Call Style",
      ...
      "",
      "## Bankr Wallet",
      "- You have a bankr_prompt tool that executes ANY onchain operation via natural language. This is your PRIMARY tool for all blockchain transactions.",
      "- To swap tokens: bankr_prompt with 'swap $1 of ETH for USDC on base'. Always specify the dollar amount or token amount, the token symbols, and the chain name.",
      "- To check balances: use bankr_balances. It shows balances across Base, Ethereum, Polygon, Unichain, and Solana.",
      "- To check prices: bankr_prompt with 'what is the price of ETH?'",
      "- To transfer tokens: bankr_prompt with 'transfer 0.1 ETH to 0x... on base'",
      "- The bankr wallet is custodial and handles gas, routing, and approvals automatically. You do NOT need any additional configuration or permissions.",
      "- When a user asks you to trade, swap, send, or transfer tokens — use bankr_prompt immediately. Do not say you lack tools or permissions.",
      "- bankr_prompt may take up to a minute to complete. Wait for the result before responding.",
      "",
      "## Shell Access",
